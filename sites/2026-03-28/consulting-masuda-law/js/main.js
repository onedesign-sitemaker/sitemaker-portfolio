...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => loading.classList.add('hide'), 1600);
  ...
  if (form) { form.addEventListener('submit', e => { e.preventDefault(); const btn = form.querySelector('button[type="submit"]'); btn.textContent = 'お申し込みを受け付けました。2営業日以内にご連絡します。'; btn.disabled = true; }); }
});
